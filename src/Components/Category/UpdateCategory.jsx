...
const UpdateCategory = () => {
    ...
    const getApiData = async () => {
        try {
            let res = await axios.get("https://api.myriccoproducts.com/api/category/" + _id)
            console.log(res);
    ...
    const postData = async (e) => {
        e.preventDefault()
        try {
            let res = await axios.put("https://api.myriccoproducts.com/api/category/" + _id, { name: name })
            if (res.status === 200) {
